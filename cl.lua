...
if not cl then cl = ffi.load("intelopencl32.dll"); end

function clGetPlatforms()
   local plist = {}

   local num_platform = ffi.new( "cl_uint[1]" )
   local platform0 = ffi.new( "cl_platform_id[?]", 2 )
--   if cl.clGetPlatformIDs( 0, nil, num_platforms ) == cl.CL_SUCCESS 
   if cl.clGetPlatformIDs( 1, platform0, num_platform ) == cl.CL_SUCCESS 
   then
      local num_platforms = num_platform[0]
	 ...
	 do
	    ...
	    do 
		  ...
		  then
		    if value_size ~= size[0] then
         assert("err: clGetPlatformInfo: size " .. value_size .. " returned size " .. size[0]) end
...
--      if cl.clGetKernelWorkGroupInfo( context, device, value_enum, 0, nil, value_size ) == cl.CL_SUCCESS
      if cl.clGetKernelWorkGroupInfo( kernel, device, value_enum, 0, nil, value_size ) == cl.CL_SUCCESS
      then
	 local value_size = tonumber( value_size[0] )
	 local value_count = value_size / ffi.sizeof(type)
	 local value = ffi.new( type.."[?]", value_count )
	 if cl.clGetKernelWorkGroupInfo( kernel, device, value_enum, value_size, value, nil ) == cl.CL_SUCCESS
	 then
	    if value_count > 1 
	    then
	       plist[key] = {}
